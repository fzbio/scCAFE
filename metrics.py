...
def exact_metrics_df(label_df, pred_df):
    merged_df = pred_df.merge(label_df, on=['chrom1', 'x1', 'y1'])
    if len(label_df) != 0:
        recall = len(merged_df) / len(label_df)
    else:
        recall = 0

    if len(pred_df) != 0:
        precision = len(merged_df) / len(pred_df)
    else:
        precision = 0
